...
local function GetBagAndSlotCompat(location)
  if not location then return nil, nil end
  if location.GetBagAndSlot then
    return location:GetBagAndSlot()
  end
  return location.bagID, location.slotIndex or location.slotID
end

local function PickupLocationCompat(location)
  local bagID, slotID = GetBagAndSlotCompat(location)
  ...
  if fromBag ~= bagID or fromSlot ~= slotID then
    if C_Item.DoesItemExist(source) then
      if not C_Item.DoesItemExist(target) then
        ...
        -- Equal stacks are interchangeable. On 3.3.5a queuing a swap between
        -- two physically different but logically identical stacks only creates
        -- churn (and can undo a previous move in the same pass).
        if BAGANATOR_335 then
          local targetLink = C_Container.GetContainerItemLink(bagID, slotID)
          local _, targetCount = C_Container.GetContainerItemInfo(bagID, slotID)
          local sourceCount = item.itemCount or 1
          targetCount = targetCount or 1
          if targetLink == item.itemLink and targetCount == sourceCount then
            return
          end
        end
...
function addonTable.Sorting.ApplyBagOrdering(bags, bagIDs, indexesToUse, bagChecks, isReverse, ignoreAtEnd, ignoreCount)
  ...
  addonTable.Sorting.AddSortKeys(oneList)
   ...
   -- final order. On the native clients GetItemGUID() is an item-stable identity,
   -- but the 3.3.5a compatibility GUID is necessarily location-based. Replacing
   -- the current ordinal with a location identity makes equal items change their
   -- tie-break key after every move, so identical stacks can keep swapping. Keep
   -- AddSortKeys()' current physical ordinal on Wrath to make equal items stable.
  ...
  -- Wrath 3.3.5a is especially sensitive to partially applying a sort while
  -- one of the involved slots is still locked. Preflight the complete move
  -- plan first: if everything is already in its destination, finish without
  -- touching the cursor; if any required slot is locked, wait before moving
  -- anything at all.
  if #moveQueue0 == 0 and #moveQueue1 == 0 then
    for _, sound in ipairs(pickupSoundIDs) do UnmuteSoundFile(sound) end
    if incomplete then
  ...
  local function LocationLocked(location)
    return C_Item.DoesItemExist(location) and C_Item.IsLocked(location)
  end
  for _, move in ipairs(moveQueue0) do
    if LocationLocked(move[1]) then
      for _, sound in ipairs(pickupSoundIDs) do UnmuteSoundFile(sound) end
      return addonTable.Constants.SortStatus.WaitingUnlock
    end
  end
  for _, move in ipairs(moveQueue1) do
    if LocationLocked(move[1]) or LocationLocked(move[2]) then
      for _, sound in ipairs(pickupSoundIDs) do UnmuteSoundFile(sound) end
      return addonTable.Constants.SortStatus.WaitingUnlock
    end
  end
  ...
  -- Modern clients lock/update slots quickly enough that the upstream sorter can
  -- consume a whole precomputed move queue in one pass. Wrath 3.3.5a updates the
  -- local bag state immediately and often does not expose that transient lock.
  -- Executing the rest of the old queue then acts on stale source locations and
  -- can undo the move we just made (A<->B followed by the stale B<->A). Do exactly
  -- one physical move per pass on 3.3.5a, wait for the cache refresh, then rebuild
  -- the plan from the new real bag state.
  if BAGANATOR_335 then
    local move = moveQueue0[1] or moveQueue1[1]
    if move then
      if not C_Item.IsLocked(move[1]) and not C_Item.IsLocked(move[2]) then
        local pickedSource = PickupLocationCompat(move[1])
        local pickedTarget = pickedSource and PickupLocationCompat(move[2])
        ClearCursor()
        moved = pickedSource and pickedTarget or false
      else
        locked = true
      end
    end
    ...
    for _, move in ipairs(moveQueue0) do
      if not C_Item.IsLocked(move[1]) then
        ...
        local pickedTarget = pickedSource and PickupLocationCompat(move[2])
        ClearCursor()
        moved = pickedSource and pickedTarget or moved
      else
        locked = true
      end
    end
