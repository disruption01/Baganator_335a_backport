...
keysMapping["sortedInvSlotID"] = function(self)
  local invSlotID = tonumber(self.invSlotID)
  if invSlotID == nil then
    -- 3.3.5a GetItemInfo returns INVTYPE_* strings. A stale/offline entry may
    -- still contain one even though the compatibility C_Item API now maps
    -- them to numeric InventoryType IDs. Treat unknown/non-equippable as 0.
    invSlotID = 0
  end
  return sortedMap.invSlotID[invSlotID] or (invSlotID + 200)
end
