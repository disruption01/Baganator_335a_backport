...
function addonTable.Sorting.OrderOneListOffline(list, sortMethod)
  ...
  -- The 3.3.5 compatibility tFilter preserves numeric keys when requested.
  -- If an empty/stale slot occurs in the middle that produces a sparse table,
  -- and Lua 5.1 table.sort() may then pass nil as one comparator operand. Build
  -- a dense sequence explicitly before sorting.
  local dense = {}
  for _, item in pairs(list or {}) do
    if type(item) == "table" and item.itemLink ~= nil then
      dense[#dense + 1] = item
    end
  end
  list = dense
