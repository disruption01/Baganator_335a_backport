...
local function ItemButtonQualityHook(frame, quality)
  if frame.bgrSimpleHooked then
    ...
    if frame.ClearNormalTexture then frame:ClearNormalTexture() elseif frame.GetNormalTexture and frame:GetNormalTexture() then frame:GetNormalTexture():SetTexture(nil) end
...
local function StyleButton(button)
  ...
  button.Right:Hide()
  button.Middle:Hide()
  if button.ClearHighlightTexture then button:ClearHighlightTexture() elseif button.GetHighlightTexture and button:GetHighlightTexture() then button:GetHighlightTexture():SetTexture(nil) end
...
local skinners = {
  ItemButton = function(frame, tags)
    ...
    if not tags.containerBag then
      ...
      frame.SlotBackground:ClearAllPoints()
      frame.SlotBackground:SetPoint("CENTER", frame, "CENTER", 0, 0)
  ...
  ButtonFrame = function(frame, tags)
    ...
    if tags.backpack then
      ...
      frame.TopButtons[1]:SetPoint("TOPLEFT", frame, "TOPLEFT", 3, -3)
    elseif tags.bank then
      frame.Character.TopButtons[1]:ClearAllPoints()
      frame.Character.TopButtons[1]:SetPoint("TOPLEFT", frame.Character, "TOPLEFT", 3, -3)
    elseif tags.guild then
      frame.ToggleTabTextButton:ClearAllPoints()
      frame.ToggleTabTextButton:SetPoint("TOPLEFT", frame, "TOPLEFT", 3, -3)
    end
  end,
