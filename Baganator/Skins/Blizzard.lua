...
local function UpdateButtonFrameVisuals(frame)
  ...
  if BAGANATOR_335 and frame.GetName and frame:GetName() then
    local name = frame:GetName()
    for _, key in ipairs({"Bg", "TopTileStreaks", "TitleBg", "BotLeftCorner", "BotRightCorner", "BottomBorder", "LeftBorder", "RightBorder", "TopRightCorner", "TopLeftCorner", "TopBorder"}) do
      if frame[key] == nil then frame[key] = _G[name .. key] end
    end
  end

  if frame.Bg then frame.Bg:SetAlpha(alpha) end
  if frame.TopTileStreaks then frame.TopTileStreaks:SetAlpha(alpha) end
  ...
  if frame.NineSlice then -- retail
    ...
    if noFrameBorders then
      if frame.Bg then frame.Bg:SetPoint("TOPLEFT", addonTable.Constants.IsClassic and 2 or 6, 0) end
      if frame.TopTileStreaks then frame.TopTileStreaks:SetPoint("TOPLEFT", 6, 0) end
    else
      if frame.Bg then frame.Bg:SetPoint("TOPLEFT", addonTable.Constants.IsClassic and 2 or 6, -21) end
  ...
  elseif frame.TitleBg then -- older classic builds
    ...
    if noFrameBorders then
      ...
      if frame.TopTileStreaks then frame.TopTileStreaks:SetPoint("TOPLEFT", 2, 0) end
    else
      if frame.Bg then
        frame.Bg:SetPoint("TOPLEFT", 2, -21)
        frame.Bg:SetPoint("BOTTOMRIGHT", -2, 2)
      end
      if frame.TopTileStreaks then frame.TopTileStreaks:SetPoint("TOPLEFT", 2, -21) end
    end
  end
end
...
local skinners = {
  ItemButton = function(frame, tags)
    if not tags.containerBag then
      if frame.SlotBackground then frame.SlotBackground:SetShown(showSlots) end
