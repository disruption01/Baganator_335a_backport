...
function BaganatorSingleViewBackpackViewMixin:AllocateBags(character)
  ...
  if self.lastBagDetails == nil or self.bagDetailsForComparison.bags == nil or not tCompare(self.bagDetailsForComparison.bags, newDetails, 15) then
    -- Commit the data state before allocating UI. On 3.3.5a a missing widget
    -- method can interrupt allocation; keeping lastBagDetails valid prevents
    -- the next refresh from incorrectly assuming nothing changed.
    ...
    self.CollapsingBags = addonTable.SingleViews.AllocateCollapsingSections(
      ...
      self.CollapsingBagSectionsPool, self.liveItemButtonPool) or self.CollapsingBags
  end
end
...
function BaganatorSingleViewBackpackViewMixin:UpdateForCharacter(character, isLive)
  ...
  if self.refreshState[addonTable.Constants.RefreshReason.ItemData] or self.lastBagDetails == nil then
  ...
  if self.refreshState[addonTable.Constants.RefreshReason.ItemData] or self.refreshState[addonTable.Constants.RefreshReason.ItemWidgets] or self.refreshState[addonTable.Constants.RefreshReason.ItemTextures] or self.refreshState[addonTable.Constants.RefreshReason.Flow] or self.refreshState[addonTable.Constants.RefreshReason.Layout] then
    ...
    local mainIndexesToUse = self.lastBagDetails and self.lastBagDetails.mainIndexesToUse or Syndicator.Constants.AllBagIndexes
    activeBag:ShowBags(bagData, character, Syndicator.Constants.AllBagIndexes, mainIndexesToUse, bagWidth)
