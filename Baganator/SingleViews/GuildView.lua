...
local function EnsureGuildInfoDialogs335(self)
  local name = self.GetName and self:GetName()
  self.LiveButtons = self.LiveButtons or {}
  self.AllFixedButtons = self.AllFixedButtons or {}
  if not self.LogsFrame and name then self.LogsFrame = _G[name .. "LogsFrame"] end
  if not self.TabTextFrame and name then self.TabTextFrame = _G[name .. "TabTextFrame"] end

  local function fallback(key)
    local frame = CreateFrame("Frame", nil, self)
    frame:Hide()
    frame.showing = PopupMode.Tab
    frame.ApplyTab = function() end
    frame.ApplyMoney = function() end
    frame.ApplyTabTitle = function() end
    frame.SetTitle = function() end
    frame.__bgr335Fallback = true
    self[key] = frame
  end
  if not self.LogsFrame then fallback("LogsFrame") end
  if not self.TabTextFrame then fallback("TabTextFrame") end

  local function ensureMoneyButton(key, text, onClick)
    if self[key] then return end
    local button = CreateFrame("Button", nil, self, "UIPanelButtonTemplate")
    button:SetSize(90, 22)
    button:SetText(text or "")
    button:SetScript("OnClick", function() onClick(self) end)
    self[key] = button
    table.insert(self.LiveButtons, button)
  end
  ensureMoneyButton("DepositButton", DEPOSIT or "Deposit", function(parent) parent:DepositMoney() end)
  ensureMoneyButton("WithdrawButton", WITHDRAW or "Withdraw", function(parent) parent:WithdrawMoney() end)

  if self.LogsFrame.__bgr335Fallback then
    if self.ToggleTabLogsButton then self.ToggleTabLogsButton:Hide() end
    if self.ToggleGoldLogsButton then self.ToggleGoldLogsButton:Hide() end
  end
  if self.TabTextFrame.__bgr335Fallback and self.ToggleTabTextButton then
    self.ToggleTabTextButton:Hide()
  end
end
...
function BaganatorSingleViewGuildViewMixin:OnLoad()
  ...
  EnsureGuildInfoDialogs335(self)
...
function BaganatorSingleViewGuildViewMixin:OnEvent(eventName, ...)
  ...
  elseif eventName == "GUILDBANKLOG_UPDATE" and self.LogsFrame and self.LogsFrame:IsVisible() then
    if self.LogsFrame.showing == PopupMode.Tab then
      self.LogsFrame:ApplyTab()
    else
      self.LogsFrame:ApplyMoney()
    end
  elseif eventName == "GUILDBANK_UPDATE_TEXT" and self.TabTextFrame and self.TabTextFrame:IsVisible() then
    self.TabTextFrame:ApplyTab()
  elseif eventName == "GUILDBANK_TEXT_CHANGED" and self.TabTextFrame and self.TabTextFrame:IsVisible() then
  ...
  elseif eventName == "PLAYER_REGEN_ENABLED" then
    addonTable.Compatibility.SetEnabled(self.WithdrawButton, self.canWithdraw)
...
function BaganatorSingleViewGuildViewMixin:HideInfoDialogs()
  if self.LogsFrame then self.LogsFrame:Hide() end
  if self.TabTextFrame then self.TabTextFrame:Hide() end
end
...
function BaganatorSingleViewGuildViewMixin:UpdateTabs(guildData)
  ...
  if #guildData.bank > 0 then
    ...
    tabButton.Icon:SetTexture("Interface\\AddOns\\Baganator\\Assets\\Everything.tga")
    tabButton.Icon:SetAlpha(1)
  ...
  for index, tabInfo in ipairs(guildData.bank) do
    ...
    addonTable.Compatibility.SetEnabled(tabButton, tabInfo.isViewable)
  ...
  if self.isLive and GetNumGuildBankTabs() < MAX_BUY_GUILDBANK_TABS and IsGuildLeader() then
    ...
    addonTable.Compatibility.SetEnabled(tabButton, true)
...
function BaganatorSingleViewGuildViewMixin:UpdateForGuild(guild, isLive)
  ...
  addonTable.Compatibility.SetEnabled(self.ToggleTabLogsButton, self.currentTab ~= 0)
  addonTable.Compatibility.SetEnabled(self.ToggleTabTextButton, self.currentTab ~= 0)
...
function BaganatorGuildTabTextTemplateMixin:ApplyTab()
  ...
  if #Syndicator.API.GetGuild(Syndicator.API.GetCurrentGuild()).bank == 0 then
    ...
    addonTable.Compatibility.SetEnabled(self.TextContainer:GetEditBox(), false)
    return
  end
  ...
  addonTable.Compatibility.SetEnabled(self.TextContainer:GetEditBox(), canEdit)
end
