...
function BaganatorSingleViewBankViewCharacterViewMixin:UpdateForCharacter(character, isLive)
  -- A brand-new character can open the bank on the same frame as Syndicator's
  -- first BANKFRAME_OPENED scan. Never build a live bank from a half-written
  -- cache; wait for BagCacheUpdate and rebuild from the completed snapshot.
  if BAGANATOR_335 and isLive and Syndicator.API.IsBagEventPending and Syndicator.API.IsBagEventPending() then
    local characterData = Syndicator.API.GetCharacter(character)
    if characterData and (characterData.__bgr335InitialBankSeen ~= true) then
      self.__bgr335WaitingForBankScan = true
      return
    end
  end
