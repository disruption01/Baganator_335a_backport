  ...
  local function ApplySearchMode()
    ...
    if oldSearch then
      if oldSearch.IsEnabled then
        addonTable.Compatibility.SetEnabled(self.CategorySearch, oldSearch:IsEnabled())
      elseif oldSearch.enabled ~= nil then
        addonTable.Compatibility.SetEnabled(self.CategorySearch, oldSearch.enabled)
      end
    end
  end
...
function BaganatorCustomiseDialogCategoriesEditorMixin:SetState(value)
  ...
  addonTable.Compatibility.SetEnabled(self.DeleteButton, tIndexOf(addonTable.CategoryViews.Constants.ProtectedCategories, value) == nil)
  ...
  if customCategories[value] then
    ...
    addonTable.Compatibility.SetEnabled(self.ItemsEditor, not category.auto)
...
function BaganatorCustomiseDialogCategoriesEditorMixin:MakeItemsEditor()
  ...
  container.Enable = function()
    addonTable.Compatibility.SetEnabled(container, true)
  end
  container.Disable = function()
    addonTable.Compatibility.SetEnabled(container, false)
  end
  container.SetEnabled = function(_, state)
    container.enabled = state
    addonTable.Compatibility.SetEnabled(addButton, state)
    addonTable.Compatibility.SetEnabled(addItemsEditBox, state)
    if addFromATTButton then
      addonTable.Compatibility.SetEnabled(addFromATTButton, state)
    end
  end
