...
local counter = 0
local function CreateDialogButton335(parent)
  local template = BAGANATOR_335 and "UIPanelButtonTemplate" or "UIPanelDynamicResizeButtonTemplate"
  local button = CreateFrame("Button", nil, parent, template)
  if BAGANATOR_335 then button:SetSize(100, 22) end
  return button
end
local function ResizeDialogButton335(button)
  if not BAGANATOR_335 and DynamicResizeButton_Resize then return DynamicResizeButton_Resize(button) end
  local text = button.GetText and button:GetText() or ""
  button:SetWidth(math.max(80, math.min(180, 24 + string.len(text or "") * 7)))
  button:SetHeight(22)
end
local function GenerateDialog()
  ...
  if BAGANATOR_335 then
    ...
    dialog:SetBackdropColor(0.03, 0.03, 0.04, 0.96)
    dialog:SetBackdropBorderColor(0.35, 0.35, 0.4, 1)
  else
  ...
  if bg.SetColorTexture then bg:SetColorTexture(0, 0, 0, 0.8) else bg:SetTexture(0, 0, 0, 0.8) end
...
function addonTable.Dialogs.ShowCopy(text)
  ...
  if not copyDialogsBySkin[currentSkinKey] then
    ...
    ResizeDialogButton335(okButton)
...
function addonTable.Dialogs.ShowEditBox(text, acceptText, cancelText, confirmCallback)
  ...
  if not editBoxDialogsBySkin[currentSkinKey] then
    ...
    dialog.acceptButton = CreateDialogButton335(dialog)
    dialog.cancelButton = CreateDialogButton335(dialog)
  ...
  ResizeDialogButton335(dialog.acceptButton)
  dialog.cancelButton:SetText(cancelText)
  ResizeDialogButton335(dialog.cancelButton)
...
function addonTable.Dialogs.ShowDualChoice(text, option1Text, option2Text, option1Callback, option2Callback)
  ...
  if not dualChoiceDialogsBySkin[currentSkinKey] then
    ...
    dialog.option1Button = CreateDialogButton335(dialog)
    dialog.option2Button = CreateDialogButton335(dialog)
    dialog.cancelButton = CreateDialogButton335(dialog)
  ...
  ResizeDialogButton335(dialog.option1Button)
  dialog.option2Button:SetText(option2Text)
  ResizeDialogButton335(dialog.option2Button)
