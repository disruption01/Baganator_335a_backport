...
itemFrame.OnUpdate = function(self, elapsed)
  ...
  if itemFrame.elapsed > 0.4 then
    for itemID in pairs(pendingItems) do
      ...
      if BAGANATOR_335 and C_Item.IsItemDataCachedByID(itemID) then
...
function addonTable.Utilities.LoadItemData(itemID, callback)
  if BAGANATOR_335 and C_Item.IsItemDataCachedByID(itemID) then callback(); return end
  pendingItems[itemID] = pendingItems[itemID] or {}
  table.insert(pendingItems[itemID], callback)
  if not BAGANATOR_335 then itemFrame:RegisterEvent("ITEM_DATA_LOAD_RESULT") end
...
if addonTable.Constants.IsRetail then
  ...
  function addonTable.Utilities.ExtractItemLevel(itemLink)
  ...
  function addonTable.Utilities.ResetItemLevelCache()
    itemLevelMap = {}
  end
end
