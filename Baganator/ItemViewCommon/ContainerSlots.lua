...
local function EnsureClassicItemButtonRegions(button)
  if not button then
    return
  end
  ...
  if name then
    button.icon = button.icon or button.Icon or button.IconTexture or _G[name .. "IconTexture"] or _G[name .. "Icon"]
    button.Icon = button.Icon or button.icon
    button.IconTexture = button.IconTexture or button.icon
    button.Count = button.Count or _G[name .. "Count"]
    button.Cooldown = button.Cooldown or _G[name .. "Cooldown"]
    button.IconBorder = button.IconBorder or _G[name .. "IconBorder"]
  end

  -- Custom 3.3.5a clients occasionally ship a reduced ItemButtonTemplate.
  -- Keep Baganator functional even if the standard $parentIconTexture region is absent.
  if not button.icon then
    button.icon = button:CreateTexture(nil, "ARTWORK")
    button.icon:SetPoint("TOPLEFT", 2, -2)
    button.icon:SetPoint("BOTTOMRIGHT", -2, 2)
    button.Icon = button.icon
    button.IconTexture = button.icon
  end
end
...
function BaganatorRetailBagSlotButtonMixin:OnLoad()
  EnsureClassicItemButtonRegions(self)
...
local Baganator335BagSlotButtonCounter = 0
local function GetBaganator335BagSlotButtonName(kind)
  Baganator335BagSlotButtonCounter = Baganator335BagSlotButtonCounter + 1
  return "Baganator335" .. (kind or "BagSlot") .. "Button" .. Baganator335BagSlotButtonCounter
end
...
function BaganatorBagSlotsContainerMixin:OnLoad()
  ...
  if self.mode == "bags" then
    GetBagSlotButton = function()
      if addonTable.Constants.IsRetail then
        return CreateFrame("Button", nil, self, "BaganatorRetailBagSlotButtonTemplate")
      else
        return CreateFrame("Button", GetBaganator335BagSlotButtonName("BagSlot"), self, "BaganatorClassicBagSlotButtonTemplate")
      end
    end
  ...
  elseif self.mode == "bank" then
    GetBagSlotButton = function()
      if addonTable.Constants.IsRetail then
        return CreateFrame("Button", nil, self, "BaganatorRetailBankButtonTemplate")
      else
        return CreateFrame("Button", GetBaganator335BagSlotButtonName("BankSlot"), self, "BaganatorClassicBankButtonTemplate")
      end
    end
  ...
  local function GetCachedBagSlotButton()
    ...
    if addonTable.Constants.IsRetail then
      return CreateFrame("Button", nil, self, "BaganatorRetailCachedItemButtonTemplate")
    else
