...
function BaganatorItemViewCommonBackpackViewMixin:OnLoad()
  if BAGANATOR_335 and addonTable.Compatibility and addonTable.Compatibility.ApplyDarkBackdrop then
    addonTable.Compatibility.ApplyDarkBackdrop(self)
  end
  Baganator335Compat.EnsureButtonFrame(self)
  if BAGANATOR_335 and Baganator335Compat and Baganator335Compat.EnsureClassicIconButton then
    -- Keep both header button groups on exactly the same 32x22 geometry.
    -- The icon itself stays centred; alignment is done by the button frame,
    -- not by pushing individual glyphs around inside their chrome.
    local headerButtons = {
      self.ToggleBankButton, self.ToggleGuildBankButton, self.ToggleAllCharacters, self.ToggleBagSlotsButton,
      self.TransferButton, self.SortButton, self.CustomiseButton,
    }
    ...
    Baganator335Compat.EnsureClassicIconButton(self.ToggleBankButton, "Interface\\AddOns\\Baganator\\Assets\\Chest.tga", 16, 0)
    Baganator335Compat.EnsureClassicIconButton(self.ToggleGuildBankButton, "Interface\\AddOns\\Baganator\\Assets\\Guild.tga", 16, -1)
    Baganator335Compat.EnsureClassicIconButton(self.ToggleAllCharacters, "Interface\\AddOns\\Baganator\\Assets\\All_Characters.tga", 17, 0)
    Baganator335Compat.EnsureClassicIconButton(self.ToggleBagSlotsButton, "Interface\\AddOns\\Baganator\\Assets\\Bags.tga", 17, 0)
    Baganator335Compat.EnsureClassicIconButton(self.CustomiseButton, "Interface\\AddOns\\Baganator\\Assets\\Cog.tga", 17, 0)
    Baganator335Compat.EnsureClassicIconButton(self.SortButton, "Interface\\AddOns\\Baganator\\Assets\\Sorting.tga", 17, 0)
    Baganator335Compat.EnsureClassicIconButton(self.TransferButton, "Interface\\AddOns\\Baganator\\Assets\\Transfer.tga", 16, 0)
  end
  if BAGANATOR_335 and self.CurrencyButton then
    self.CurrencyButton.Icon = self.CurrencyButton.Icon or (self.CurrencyButton.GetName and _G[self.CurrencyButton:GetName() .. "Icon"])
    if not self.CurrencyButton.Icon then self.CurrencyButton.Icon=self.CurrencyButton:CreateTexture(nil,"ARTWORK"); self.CurrencyButton.Icon:SetSize(14,14); self.CurrencyButton.Icon:SetPoint("CENTER") end
    self.CurrencyButton.Icon:SetTexture("Interface\\AddOns\\Baganator\\Assets\\Currency.tga"); self.CurrencyButton.Icon:Show()
    self.CurrencyButton.tooltipHeader = addonTable.Locales.CURRENCIES
    self.CurrencyButton:SetScript("OnClick", function() addonTable.CallbackRegistry:TriggerEvent("CurrencyPanelToggle") end)
  end
  ...
  self.TopButtons[1]:SetPoint("TOPLEFT", self, "TOPLEFT", addonTable.Constants.ButtonFrameOffset + 3, -3)
...
function BaganatorItemViewCommonBackpackViewMixin:OnFinished(forceResize)
  if self.refreshState[addonTable.Constants.RefreshReason.Layout] or forceResize then
    ...
    -- On Wrath 3.3.5a GetTop() can transiently be nil during the first
    -- layout pass for a freshly-created character. Avoid arithmetic until
    -- both anchors have resolved; a later layout refresh will recalculate it.
    local externalVerticalSpacing = 0
    local bagSlotsHeight = self.BagSlots:GetHeight() or 0
    local bagSlotsTop, selfTop = self.BagSlots:GetTop(), self:GetTop()
    if bagSlotsHeight > 0 and bagSlotsTop and selfTop then
      externalVerticalSpacing = bagSlotsTop - selfTop
    end
...
function BaganatorItemViewCommonBackpackViewMixin:UpdateGuildButton()
  ...
  addonTable.Compatibility.SetEnabled(self.ToggleGuildBankButton, guildName ~= nil and Syndicator.API.GetGuild(guildName) ~= nil)
