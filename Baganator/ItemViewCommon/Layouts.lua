...
local function FlowButtonsRows(self, rowWidth)
  ...
  for _, button in ipairs(self.buttons) do
    if BAGANATOR_335 then
      -- A pooled Wrath button may still have anchors from a previous bag/bank
      -- layout. Always start from one anchor or it can stretch across the view.
...
function BaganatorLiveBagLayoutMixin:UpdateLockForItem(bagID, slotID)
  ...
  if itemButton then
    ...
    SetItemButtonDesaturated(itemButton, locked or (itemButton.BGR and itemButton.BGR.persistIconGrey) or false)
  end
end
...
local function InitializeCategoryEmptySlot(button, details)
  ...
  if details then
    ...
    button.bagTypeIcon:SetAlpha(1)
    button.bagTypeIcon:Show()
    button.tooltipHeader = details.tooltipHeader
  else
    button.bagTypeIcon:SetTexture(nil)
    button.bagTypeIcon:Hide()
...
function BaganatorBagDividerMixin:OnLoad()
  if not self.Divider then
    -- The pool creates this virtual template anonymously on 3.3.5a, so the
    -- converted `$parentDivider` texture cannot be found through _G[name].
    -- There is only one texture region in this template; expose it explicitly.
  ...
  if not self.Divider then
    self.Divider = self:CreateTexture(nil, "BACKGROUND")
  end

  if BAGANATOR_335 then
    -- Atlas APIs/textures used by modern Classic are unavailable in Wrath.
    -- A simple one-pixel divider preserves the layout semantics cleanly.
    self.Divider:SetTexture(1, 1, 1, 0.28)
    self.Divider:SetHeight(1)
    self.Divider:ClearAllPoints()
    self.Divider:SetPoint("TOPLEFT", 0, 0)
    self.Divider:SetPoint("TOPRIGHT", 0, 0)
