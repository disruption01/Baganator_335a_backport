...
local function NormalizeTooltipAnchor335(anchor)
  if BAGANATOR_335 and anchor and not anchor.SetScript and anchor.GetParent then
    anchor = anchor:GetParent()
  end
  if not anchor or not anchor.GetObjectType then
    anchor = UIParent
  end
  return anchor
end
...
function addonTable.ShowGoldSummaryRealm(anchor, point)
  anchor = NormalizeTooltipAnchor335(anchor)
  GameTooltip:Hide()
