...
function BaganatorItemViewCommonBankViewCharacterViewMixin:OnLoad()
  if BAGANATOR_335 and self.GetRegions then
    for _, region in ipairs({self:GetRegions()}) do
      if region and region.GetObjectType and region:GetObjectType() == "FontString" then
        local text = region.GetText and region:GetText()
        if text == addonTable.Locales.BANK_DATA_MISSING_HINT or (type(text) == "string" and text:find("%s", 1, true)) then
          region:SetText("")
          region:Hide()
        end
      end
  ...
  if not self.BankMissingHint then
    self.BankMissingHint = self:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    self.BankMissingHint:SetPoint("CENTER", self, "CENTER", 0, 0)
    self.BankMissingHint:SetWidth(360)
    self.BankMissingHint:SetJustifyH("CENTER")
    self.BankMissingHint:Hide()
  end
...
function BaganatorItemViewCommonBankViewCharacterViewMixin:UpdateForCharacter(character, isLive)
  ...
  local missingBankData = (not self.isLive) and (characterData.bank[1] == nil or #characterData.bank[1] == 0)
  self.BankMissingHint:SetShown(missingBankData)
...
function BaganatorItemViewCommonBankViewCharacterViewMixin:OnFinished(forceResize)
  ...
  if self.refreshState[addonTable.Constants.RefreshReason.Layout] or forceResize then
    ...
    -- On Wrath 3.3.5a GetTop/GetBottom can transiently return nil while the
    -- first live bank layout is being anchored (notably empty bank -> first
    -- deposited item). Never do arithmetic on unresolved anchors.
    local externalVerticalSpacing = 0
    local bagSlotsTop, selfTop = self.BagSlots:GetTop(), self:GetTop()
    if self.BagSlots:GetHeight() > 0 and bagSlotsTop and selfTop then
      externalVerticalSpacing = externalVerticalSpacing + (bagSlotsTop - selfTop)
    end

    local parent = self:GetParent()
    local firstTab = parent.Tabs and parent.Tabs[1]
    local parentBottom = parent:GetBottom()
    local tabBottom = firstTab and firstTab:GetBottom()
    if firstTab and firstTab:IsShown() and parentBottom and tabBottom then
      externalVerticalSpacing = externalVerticalSpacing + (parentBottom - tabBottom + 5)
    end

    local parentScale = parent:GetScale() or 1
    if parentScale == 0 then parentScale = 1 end
    local extraCurrencyHeight = (self.CurrencyWidget and self.CurrencyWidget.GetExtraHeight and self.CurrencyWidget:GetExtraHeight()) or 0

    self:SetSize(
      (self.Container:GetWidth() or 0) + sideSpacing * 2 + addonTable.Constants.ButtonFrameOffset - 2,
      math.min((self.Container:GetHeight() or 0) + 50 + searchSpacing + buttonPadding + extraCurrencyHeight, UIParent:GetHeight() / parentScale - externalVerticalSpacing)
    )

    self:UpdateScroll(73 + buttonPadding + externalVerticalSpacing, parentScale)
  end
end
