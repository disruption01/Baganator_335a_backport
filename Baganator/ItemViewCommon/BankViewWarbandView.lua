...
function BaganatorItemViewCommonBankViewWarbandViewMixin:UpdateCurrencies()
  ...
  if self.isLive then
    addonTable.Compatibility.SetEnabled(self.DepositMoneyButton, C_Bank.CanDepositMoney(Enum.BankType.Account))
    addonTable.Compatibility.SetEnabled(self.WithdrawMoneyButton, C_Bank.CanWithdrawMoney(Enum.BankType.Account))
  end
end
