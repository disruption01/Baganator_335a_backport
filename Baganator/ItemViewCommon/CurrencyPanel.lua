...
function BaganatorCurrencyPanelMixin:OnLoad()
  ...
  self.searchBox = CreateFrame("EditBox", nil, self, BAGANATOR_335 and "InputBoxTemplate" or "SearchBoxTemplate")
  ...
  self.scrollBox = CreateFrame("Frame", nil, self, BAGANATOR_335 and nil or "WowScrollBoxList")
  local scrollBar = CreateFrame(BAGANATOR_335 and "Slider" or "Frame", nil, self, BAGANATOR_335 and "UIPanelScrollBarTemplate" or "MinimalScrollBar")
  ...
  if BAGANATOR_335 then Baganator335Compat.InitScrollBoxListWithScrollBar(self.scrollBox, scrollBar, view) else ScrollUtil.InitScrollBoxListWithScrollBar(self.scrollBox, scrollBar, view) end
...
function BaganatorCurrencyPanelMixin:SetupRow(row, details)
  ...
  row.arrowIcon:Hide()
  row.warbandIcon:Hide()
  addonTable.Compatibility.SetEnabled(row, not details.disabled)
  ...
  elseif details.type == "text" then
    row:SetNormalFontObject(GameFontNormal)
    row:SetDisabledFontObject(GameFontNormal)
...
function addonTable.ItemViewCommon.GetCurrencyPanel(frameName)
  if BAGANATOR_335 then
    -- Stock 3.3.5a has the old Currency/Token API, but none of the modern
    -- ScrollBox/warband machinery used by the current Classic panel. Build a
    -- small native panel instead so the footer Currency button is fully usable.
    ...
    frame:SetTitle(addonTable.Locales.CURRENCIES or CURRENCY or "Currencies")
    frame:SetScript("OnDragStart", function(self) self:StartMoving() end)
    frame:SetScript("OnDragStop", function(self) self:StopMovingOrSizing() end)

    local scroll = CreateFrame("ScrollFrame", frameName .. "ScrollFrame", frame, "UIPanelScrollFrameTemplate")
    scroll:SetPoint("TOPLEFT", 14, -34)
    scroll:SetPoint("BOTTOMRIGHT", -30, 14)
    local child = CreateFrame("Frame", frameName .. "ScrollChild", scroll)
    child:SetWidth(310)
    child:SetHeight(1)
    scroll:SetScrollChild(child)
    frame.currencyRows335 = {}

    local function ensureRow(index)
      local row = frame.currencyRows335[index]
      ...
      frame.currencyRows335[index] = row
      return row
    end

    local legacyCurrencyTextures335 = {
      ["Badge of Justice"] = "Interface\\Icons\\pvecurrency-justice",
      ["Emblem of Heroism"] = "Interface\\Icons\\pvecurrency-emblemofheroism",
      ["Emblem of Valor"] = "Interface\\Icons\\pvecurrency-emblemofvalor",
      ["Emblem of Conquest"] = "Interface\\Icons\\pvecurrency-emblemofconquest",
      ["Emblem of Triumph"] = "Interface\\Icons\\pvecurrency-emblemoftriumph",
      ["Emblem of Frost"] = "Interface\\Icons\\pvecurrency-emblemoffrost",
    }
    local legacyCurrencyItemIDs335 = {
    ...
    local function rebuild()
      ...
      local used = 0
      for i = 1, total do
        local ok, name, isHeader, isExpanded, isUnused, isWatched, count, icon = pcall(GetCurrencyListInfo, i)
        if ok and name and not isUnused then
          used = used + 1
          ...
          if isHeader then
            row.icon:Hide()
            row.name:ClearAllPoints(); row.name:SetPoint("LEFT", 4, 0); row.name:SetPoint("RIGHT", -8, 0)
            row.name:SetText((isExpanded and "- " or "+ ") .. name)
            row.name:SetTextColor(1, 0.82, 0, 1)
            row.count:SetText("")
            local currencyIndex = i
            local expanded = isExpanded
            row:SetScript("OnClick", function()
              if ExpandCurrencyList then ExpandCurrencyList(currencyIndex, not expanded) end
              rebuild()
            end)
          else
            row.icon:Show()
            local itemID = legacyCurrencyItemIDs335[name]
            local itemIcon = itemID and GetItemIcon and GetItemIcon(itemID) or nil
            if name == "Badge of Justice" then
              -- Badge of Justice uses this icon in the classic clients.
              icon = "Interface\\Icons\\Spell_Holy_ChampionsBond"
            elseif itemIcon then
              icon = itemIcon
            elseif type(icon) ~= "string" or icon == "" then
              icon = legacyCurrencyTextures335[name]
            end
            row.icon:SetTexture(icon or "Interface\\Icons\\INV_Misc_QuestionMark")
            row.icon:SetTexCoord(0, 1, 0, 1)
            row.icon:SetVertexColor(1, 1, 1, 1)
            row.name:ClearAllPoints(); row.name:SetPoint("LEFT", row.icon, "RIGHT", 6, 0); row.name:SetPoint("RIGHT", -66, 0)
            row.name:SetText(name)
            row.name:SetTextColor(1, 1, 1, 1)
            row.count:SetText(tostring(count or 0))
            row:SetScript("OnClick", nil)
          end
        end
      end
      for i = used + 1, #frame.currencyRows335 do frame.currencyRows335[i]:Hide() end
      child:SetHeight(math.max(1, used * 26))
    end
