...
function BaganatorCurrencyWidgetMixin:OnLoad()
  ...
  local function UpdateMoneyDisplay()
    -- On 3.3.5a FontStrings cannot own tooltips. Anchor to the tiny mouse hit
    -- frame surrounding the money text instead of the whole CurrencyWidget,
    -- otherwise GameTooltip can appear in the middle of the screen.
    local anchor = self.moneyHitFrame335 or self.Money
    if IsShiftKeyDown() then
      addonTable.ShowGoldSummaryAccount(anchor, "ANCHOR_TOP")
    else
      addonTable.ShowGoldSummaryRealm(anchor, "ANCHOR_TOP")
    end
  end
  local moneyRegion = self.Money
  if BAGANATOR_335 and (not moneyRegion.SetScript) then
    local moneyHitFrame = CreateFrame("Frame", nil, self)
    moneyHitFrame:SetAllPoints(moneyRegion)
    moneyHitFrame:EnableMouse(true)
    self.moneyHitFrame335 = moneyHitFrame
    moneyRegion = moneyHitFrame
  end

  moneyRegion:SetScript("OnEnter", function()
    UpdateMoneyDisplay()
    frame:RegisterEvent("MODIFIER_STATE_CHANGED")
    frame:SetScript("OnEvent", UpdateMoneyDisplay)
  end)

  moneyRegion:SetScript("OnLeave", function()
