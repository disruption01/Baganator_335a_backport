...
Baganator.API.RegisterCornerWidget(addonTable.Locales.ITEM_LEVEL, "item_level", function(ItemLevel, details)
  if HasItemLevel(details) and not (C_Item.IsCosmeticItem and C_Item.IsCosmeticItem(details.itemLink)) then
    ...
    -- On 3.3.5a starter gear commonly has item level 1. Rendering that in
    -- the top-left corner looks exactly like a bogus stack count on
    -- non-stackable items, so suppress the uninformative level-1 marker.
    if BAGANATOR_335 and (tonumber(details.itemLevel) or 0) <= 1 then
      ItemLevel:SetText("")
      return false
    end
...
Baganator.API.RegisterCornerWidget(addonTable.Locales.QUANTITY, "quantity", function(_, details)
  ...
  local count = itemButton.Count
  if not count and itemButton.GetName then
    local name = itemButton:GetName()
    if name then count = _G[name .. "Count"] or _G[name .. "CountText"] end
  end
  if not count then
    count = itemButton:CreateFontString(nil, "OVERLAY", "NumberFontNormal")
    count:SetPoint("BOTTOMRIGHT", itemButton, "BOTTOMRIGHT", -2, 2)
