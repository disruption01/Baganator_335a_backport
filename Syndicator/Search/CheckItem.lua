...
local PVP_PATTERN = PVP_ITEM_LEVEL_TOOLTIP and PVP_ITEM_LEVEL_TOOLTIP:gsub("%%d", ".*") or nil
local function PvPCheck(details)
  -- Wrath has no "PvP item level" tooltip line; this keyword is therefore
  -- intentionally unsupported instead of trying to match unrelated text.
  if not PVP_PATTERN then return false end
  if not addonTable.Utilities.IsEquipment(details.itemLink) then
    return false
  end
...
local classRestrictionsPattern = ITEM_CLASSES_ALLOWED and ITEM_CLASSES_ALLOWED:gsub("%%s", ".+") or "\001"
local multipleUniquesPattern = ITEM_UNIQUE_MULTIPLE and ITEM_UNIQUE_MULTIPLE:gsub("%%d", ".+") or "\001"
...
AddKeywordLocalised("KEYWORD_ARMOR_STAT", GetGemStatCheck(STAT_ARMOR or ARMOR or "Armor"), addonTable.Locales.GROUP_STAT)
if addonTable.Constants.IsClassic then
  local resistances = {
    "holy resistance",
    "fire resistance",
...
function addonTable.Search.InitializeSearchEngine()
  ...
  if SYNDICATOR_335 then
    classesToCheck[17] = nil -- battle pets
    classesToCheck[18] = nil -- WoW token
    classesToCheck[19] = nil -- profession equipment
    classesToCheck[20] = nil -- housing
  end
  if addonTable.Constants.IsEra then
  ...
  if C_PetJournal and not SYNDICATOR_335 then
    local petsToCheck = {
      "dragonkin",
      "flying",
      "undead",
  ...
  if not SYNDICATOR_335 and C_Item.GetItemClassInfo(20) then -- Housing
    local decorToCheck = {
      [0] = "decor",
      [1] = "housing dye",
      [2] = "room",
