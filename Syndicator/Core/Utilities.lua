...
itemFrame.elapsed = 0
if not SYNDICATOR_335 then itemFrame:RegisterEvent("ITEM_DATA_LOAD_RESULT") end
...
itemFrame.OnUpdate = function(self, elapsed)
  ...
  if itemFrame.elapsed > 0.4 then
    for itemID in pairs(pendingItems) do
      ...
      if SYNDICATOR_335 and C_Item.IsItemDataCachedByID(itemID) then
        local callbacks = pendingItems[itemID]
        pendingItems[itemID] = nil
...
function addonTable.Utilities.LoadItemData(itemID, callback)
  ...
  if SYNDICATOR_335 and C_Item.IsItemDataCachedByID(itemID) then callback(); return end
...
-- 3.3.5a does not understand the modern |A:atlas:...|a text markup; it
-- renders the atlas name literally in tooltips. Use achievement icon textures
-- that are shipped by the Wrath client and classic |T...|t markup instead.
local wrathRaceTextureNames = {
  human = "Human",
  orc = "Orc",
  dwarf = "Dwarf",
  nightelf = "NightElf",
...
local wrathGenderNames = {
  [1] = "Male", -- unknown: use a stable fallback rather than raw atlas text
  [2] = "Male",
  [3] = "Female",
}
...
function addonTable.Utilities.GetCharacterIcon(race, sex)
  ...
  race = race:lower()
  local correctedRace = raceCorrections[race] or race

  if addonTable.Constants.IsWrath then
    local raceName = wrathRaceTextureNames[correctedRace]
    local genderName = wrathGenderNames[sex] or "Male"
    if not raceName then
      return ""
    end
    return "|TInterface\\Icons\\Achievement_Character_" .. raceName .. "_" .. genderName .. ":13:13|t"
  end

  return "|A:"..prefix.."-" .. correctedRace .. "-" .. (genders[sex] or "unknown") .. ":13:13|a"
end
